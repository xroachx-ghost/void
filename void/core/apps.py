...
class AppManager:
    ...
    def force_stop_app(device_id: str, package: str) -> bool:
        ...
        code, _, _ = SafeSubprocess.run(['adb', '-s', device_id, 'shell', 'am', 'force-stop', package])
        return code == 0

    @staticmethod
    def grant_permission(device_id: str, package: str, permission: str) -> bool:
        """Grant permission to app"""
        code, _, _ = SafeSubprocess.run(['adb', '-s', device_id, 'shell', 'pm', 'grant', package, permission])
        return code == 0

    @staticmethod
    def revoke_permission(device_id: str, package: str, permission: str) -> bool:
        """Revoke permission from app"""
        code, _, _ = SafeSubprocess.run(['adb', '-s', device_id, 'shell', 'pm', 'revoke', package, permission])
        return code == 0

    @staticmethod
    def launch_app(device_id: str, package: str) -> bool:
        """Launch application by package name"""
        code, _, _ = SafeSubprocess.run(['adb', '-s', device_id, 'shell', 'monkey', '-p', package, '-c', 'android.intent.category.LAUNCHER', '1'])
        return code == 0
    ...
    def get_app_info_detailed(device_id: str, package: str) -> Dict[str, Any]:
        ...
        if code == 0:
            ...
            # Get size
            code, stdout, _ = SafeSubprocess.run(['adb', '-s', device_id, 'shell', 'du', '-h', apk_path])
            if code == 0:
