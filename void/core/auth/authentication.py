...
class AuthenticationManager:
    ...
    def authenticate(self, username: str, password: str, ip_address: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        ...
        if password_hash != stored_hash:
            ...
            self._audit_log(username, 'login_failed', False, ip_address, f'Invalid password (attempt {failed_attempts})')
            conn.close()
            return False, None
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, 1)
        ''', (user_id, session_token, datetime.now().isoformat(), expires_at, datetime.now().isoformat(), ip_address))
    ...
    def _audit_log(self, username: str, action: str, success: bool, ip_address: Optional[str] = None, details: Optional[str] = None):
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (username, action, 1 if success else 0, ip_address, datetime.now().isoformat(), details))
